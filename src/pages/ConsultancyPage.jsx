                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {currentConsultancy.map((item, idx) => (
                        <motion.div
                            ...
                            key={item.id}
                            className="group bg-white rounded-[2rem] border border-slate-200/60 shadow-xl shadow-blue-900/5 hover:border-[#ffc107] transition-all p-6 flex flex-col"
                        >
                            <div className="flex justify-between items-start mb-6">
                                <div className="w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center text-[#ffc107] group-hover:bg-[#0f172a] group-hover:text-white transition-all duration-500">
                                    {React.cloneElement(item.icon, { size: 18 })}
                                </div>
                                <div className="text-right">
                                    <span className="inline-flex items-center px-2.5 py-0.5 bg-[#224292] text-white text-[10px] font-black uppercase rounded-full tracking-widest mb-2 italic">
                                        {item.status}
                                    </span>
                                    <div className="flex items-center justify-end gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none">
                                        <Calendar size={10} className="text-[#ffc107]" /> {item.year}
                                    </div>
                                </div>
                            </div>

                            <div className="flex-1">
                                <h3 className="text-base font-black text-[#224292] uppercase leading-tight mb-4 transition-colors italic">
                                    "{item.title}"
                                </h3>
                                
                                <div className="space-y-2.5 mb-6">
                                    <div className="flex items-start gap-4 p-3.5 bg-slate-50 rounded-2xl border border-slate-100/50 group-hover:bg-white transition-colors">
                                        <Building2 size={14} className="text-slate-300 mt-0.5 shrink-0" />
                                        <div>
                                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Industry Partner</p>
                                            <p className="text-[13px] font-bold text-black uppercase tracking-tight">{item.industry}</p>
                                        </div>
                                    </div>
                                    
                                    <div className="flex items-start gap-4 p-3.5 bg-slate-50 rounded-2xl border border-slate-100/50 group-hover:bg-white transition-colors">
                                        <Handshake size={14} className="text-[#ffc107] mt-0.5 shrink-0" />
                                        <div>
                                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Domain Expertise</p>
                                            <p className="text-[13px] font-bold text-black uppercase tracking-tight">{item.domain}</p>
                                        </div>
                                    </div>

                                    {item.pi && (
                                        <div className="flex items-start gap-4 p-3.5 bg-slate-50 rounded-2xl border border-slate-100/50 group-hover:bg-white transition-colors">
                                            <Users size={14} className="text-slate-300 mt-0.5 shrink-0" />
                                            <div>
                                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Consultants (PI / Co-PI)</p>
                                                <p className="text-[11px] font-bold text-black uppercase tracking-tight leading-relaxed">{item.pi}</p>
                                            </div>
                                        </div>
                                    )}
                                </div>
                            ...
                            <div className="pt-5 border-t border-slate-100 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    <div className="w-9 h-9 rounded-full bg-[#ffc107]/10 flex items-center justify-center text-[#ffc107]">
                                        <BadgeDollarSign size={18} />
                                    </div>
                                    <div>
                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[2px]">Revenue Generated</p>
                                        <p className="text-base font-black text-[#224292] tracking-tight group-hover:text-[#ffc107] transition-colors italic">₹ {item.revenue}</p>
                                    </div>
                                </div>
                            </div>
                        </motion.div>
